...
const APP_CONFIG = {
  ...
  contactLineUrl: "https://line.me/ti/p/~@703hkgpk", 
};
...
// 🗺️ Leaflet 互動地圖組件
// ==========================================
const InteractiveMap = ({ shops, onMarkerClick, villageData, language }) => {
  ...
  const getCategoryEmoji = (categories) => {
    ...
    if (cat.includes('gift') || cat.includes('伴手禮')) return '🎁';
    return '📍';
  };
    ...
    loadLeaflet().then((L) => {
      ...
      shops.forEach(shop => {
        if (shop.lat && shop.lng) {
          ...
          const emoji = getCategoryEmoji(shop.categories);
...
const ImageCarousel = ({ images, credit, onClick }) => {
...
export default function App() {
  ...
  const handleGetLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setSortBy('distance');
        setCurrentView('home');
        setViewMode('list'); // 定位後預設切換回列表方便查看距離
        setLoading(false);
      },
      (error) => {
  ...
  const processedShops = useMemo(() => {
    ...
    if (filterEV) result = result.filter(shop => shop.hasEVCharging);
    ...
    <div className="relative w-full min-h-[100dvh] font-sans text-gray-800 overflow-x-hidden animate-fade-in bg-gray-50">
      ...
      <style>
        {`
          ...
          .leaflet-tooltip { background: rgba(30, 30, 30, 0.9) !important; color: white !important; border: none !important; box-shadow: 0 4px 6px rgba(0,0,0,0.3) !important; border-radius: 8px !important; }
          .leaflet-tooltip-top:before { border-top-color: rgba(30, 30, 30, 0.9) !important; }
        `}
      </style>
      ...
      <div className="relative z-10 w-full pb-32">
              ...
              <div className="px-6 mb-8">
                <div className="flex md:flex-wrap justify-start md:justify-center gap-3 md:gap-5 overflow-x-auto pb-4 no-scrollbar max-w-4xl mx-auto px-2">
                  {availableCategories.map((catKey) => {
                    ...
                    return (
                      <button key={catKey} onClick={() => setActiveCategory(catKey)} className={`flex flex-col items-center justify-center min-w-[70px] h-16 md:minw-[90px] md:h-20 rounded-2xl transition-all duration-300 border ${ isActive ? 'transform scale-105 border-transparent' : 'bg-white/95 backdrop-blur-sm text-gray-500 border-gray-200 hover:bg-white shadow-sm hover:shadow-md hover:-translate-y-1' }`} style={isActive ? { backgroundColor: currentPrimaryColor, color: currentBadgeColor, boxShadow: `0 10px 20px -5px ${hexToRgba(currentPrimaryColor, 0.5)}` } : {}}>
         ...
         <div className="px-6 mb-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 max-w-7xl mx-auto w-full border-b border-gray-200/50 pb-4">
              ...
              <div className="flex flex-wrap items-center justify-start md:justify-end gap-2">
                ...
                {filterEV && <span className="text-xs font-bold px-2.5 py-1.5 rounded-lg flex items-center gap-1 border bg-green-50 text-green-700 border-green-200 shadow-sm"><Zap size={14} /> 電車充電</span>}
         ...
         <div className="px-6 w-full">
            {loading ? (
              ...
              viewMode === 'list' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 max-w-7xl mx-auto pb-10 animate-fade-in">
                      ...
                      <div key={shop.id} className="group relative bg-white/95 backdrop-blur-md rounded-[28px] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:shadow-[0_15px_40px_rgb(0,0,0,0.12)] transition-all duration-300 border border-white flex flex-col h-full hover:-translate-y-1">
                        ...
                        <div className="p-6 cursor-pointer flex flex-col flex-1" onClick={() => setSelectedShop(shop)}>
                          ...
                          <div className="flex flex-wrap items-center gap-2.5 mb-4">
                              ...
                              {shop.hasEVCharging && <span className="text-[11px] font-extrabold bg-green-50 text-green-700 border border-green-200 px-2.5 py-1 rounded-lg shadow-sm flex items-center gap-1"><Zap size={12} /> 電車充電</span>}
