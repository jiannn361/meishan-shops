...
const translations = {
  zh: {
    ...
    searchPlaceholder: '搜尋關鍵字或服務(例如: 茶)...', all: '全部',
  ...
  en: {
    ...
    searchPlaceholder: 'Search keyword or services...', all: 'All',
    ...
    distance: 'Dist', shopsCount: 'shops', loading: 'Almost there, please wait...',
    noFavorites: 'You have no favorites yet!', noShops: 'Oops, no shops match your criteria.',
    ...
    quickFilter: 'Quick Filter', onlyOpenNow: 'Open Now Only', onlyElevator: 'Elevator/Accessible Only',
      ...
      <div className="relative w-full max-w-sm bg-white rounded-t-3xl sm:rounded-3xl p-6 space-y-6 animate-slide-up">
        ...
        <div className="text-center pt-2">
          <div className="w-20 h-20 mx-auto rounded-full overflow-hidden border-4 shadow-lg mb-4 flex items-center justify-center" style={{ borderColor: hexToRgba(currentPrimaryColor, 0.2), backgroundColor: hexToRgba(currentPrimaryColor, 0.05) }}>
            {userProfile?.pictureUrl ? <img src={userProfile.pictureUrl} alt="User" className="w-full h-full object-cover" /> : (
              <div className="relative w-full h-full flex items-center justify-center"><Mountain size={40} className="relative z-10" strokeWidth={1.5} style={{ color: currentPrimaryColor }} /><div className="absolute bottom-0 w-full h-1/3" style={{ backgroundColor: hexToRgba(currentPrimaryColor, 0.2) }}></div></div>
            )}
          </div>
...
export default function App() {
    ...
    const fetchAirtableData = async () => {
      ...
      try {
        ...
        const processedShops = allRecords.map(record => {
            ...
            else categories = String(rawCat).split(/[,，/|、]/).map(c => c.trim()).filter(Boolean);

            let images = [];
            const rawImg = f['images'] || f['image'] || f['圖片'] || f['圖片網址'] || f['Images'];
            if (Array.isArray(rawImg)) images = rawImg.map(img => img.url || img);
            else if (rawImg) images = String(rawImg).split(/[,，]/).map(s => s.trim());
            ...
            else if (rawSvc) services = String(rawSvc).split(/[,，]/).map(s => s.trim());
            ...
            return {
              ...
              bookings: shopBookings, hours: f['hours'] || f['Hours'] || f['營業時間'] || '',
  ...
  return (
    <div className="min-h-[100dvh] bg-gray-50 text-gray-800 font-sans flex justify-center overflow-hidden animate-fade-in relative">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                          ...
                          {!hideCardHours && (
                            <div className="flex items-center gap-1.5 text-xs px-2 py-1 rounded-md border border-gray-100 bg-gray-50 text-gray-500 max-w-full overflow-hidden">
                              ...
                              <span className="truncate">{shop.hours.trim().toLowerCase() === 'google' ? t('googleInfo') : shop.hours.trim().toLowerCase() === 'fb' ? t('fbAnnouncement') : shop.hours.split('|')[0]}</span>
                            </div>
                          )}
                      </div>
      ...
      {/* 💥 將所有彈窗與側邊欄移至最外層，徹底解決 z-index 被底部導航列遮擋的問題 💥 */}
      {isSidebarOpen && (
      ...
      <UserModal showUserModal={showUserModal} setShowUserModal={setShowUserModal} userProfile={userProfile} t={t} APP_CONFIG={APP_CONFIG} currentPrimaryColor={currentPrimaryColor} currentDarkColor={currentDarkColor} favorites={favorites} setFavorites={setFavorites} />
    </div>
  );
}